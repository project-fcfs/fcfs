async function createOrder() {
    ...
    console.log(buyNow);
    const productIds = [buyNow.productId];
    const count = buyNow.count

    const url = `/product-service/products/cart?ids=${productIds.join(',')}`;
    const items = await fetchApi(url, 'GET');
    const orderData = items.data;

    // 주문 요약 표시
    displayOrderSummary(orderData, productIds, count);
    
}

function displayOrderSummary(order, productIds, count) {
    ...
    container.innerHTML = `
        ...
        <ul>
            ${order.map(product => {
                const productTotal = product.price * count; // 각 상품의 총 가격 계산
